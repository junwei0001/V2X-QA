...
"""IS-focused refinement for V2X-MoE on Qwen3-VL.

Goal:
1) Improve IS1-IS4 accuracy efficiently.
...
4) Preserve the gains you already obtained from CO refinement.

Open-source note:
This repository does not redistribute the original V2X-Seq-SPD infrastructure-side
images. Users must download the raw images from the official source separately and
...
class Config:
    ...
    # Recommended: point this to your current best checkpoint, e.g. the CO-boosted checkpoint
    base_checkpoint_dir = OUTPUT_ROOT / "stage2_co" / "epoch_2"
    output_dir = OUTPUT_ROOT / "stage3_is"
    ...
    # Speed / memory on single RTX 4090
    image_max_side = 560
    ...
    use_task_hint = True
    use_view_hint = True
    use_is_instruction = True

    is_task_balanced_sampling = True
    is_balance_alpha = 1.0

    # Optional extra emphasis on harder IS tasks
    extra_task_weight = {
        "IS1": 1.0,
        "IS2": 1.0,
        "IS3": 1.2,
...
logger = logging.getLogger("v2x_moe_train_mcqa_qwen3_is_boost")

IS_TASKS = ["IS1", "IS2", "IS3", "IS4"]
...
def build_is_prompt(task_id: str, question: str, options: Dict[str, str]) -> str:
    lines = []
    lines.append("You are solving a multiple-choice autonomous driving question.")
    if args.use_view_hint:
        lines.append("The image is an infrastructure-side roadside-unit view.")
    if args.use_task_hint:
        lines.append(f"Task ID: {task_id}.")
    if args.use_is_instruction:
        lines.append("Use the global intersection layout, traffic participants, and long-range cues visible from the roadside view.")
        lines.append("Prefer evidence directly visible in the infrastructure view.")
...
# dataset
# ============================================================
class ISOnlyTrainDataset(Dataset):
    def __init__(self, jsonl_dir: str, processor: AutoProcessor):
        self.samples: List[Dict] = []
        self.processor = processor
        self.root_is = args.image_root_is

        logger.info("[IS-BOOST] Loading only IS1-IS4 jsonl files into RAM ...")
        self._load_and_cache(jsonl_dir)
        logger.info(f"[IS-BOOST] Loaded {len(self.samples)} valid IS training samples.")

        self.task_counts = Counter(s["task_id"] for s in self.samples)
        logger.info(f"[IS-BOOST] Task counts: {dict(self.task_counts)}")

    def _load_and_cache(self, jsonl_dir: str) -> None:
        jsonl_files = sorted(glob.glob(os.path.join(jsonl_dir, "IS*.jsonl")))
        if not jsonl_files:
            raise FileNotFoundError(f"No IS*.jsonl files found in: {jsonl_dir}")
    ...
    def __getitem__(self, idx: int):
        ...
        img_is = robust_load_image(data["img_path"])

        user_content = [{"type": "image", "image": img_is}]
        prompt = build_is_prompt(data["task_id"], data["question"], options)
...
def setup_model_from_checkpoint():
    ...
    for name, p in model.named_parameters():
        p.requires_grad = ("is_expert" in name)

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())
    logger.info(f"Trainable params (IS only): {trainable:,} / {total:,} ({100.0 * trainable / total:.4f}%)")
    return model
...
# dataloader helpers
# ============================================================
def build_task_balanced_sampler(dataset: ISOnlyTrainDataset):
    if not args.is_task_balanced_sampling:
        return None
    ...
    for sample in dataset.samples:
        ...
        c = counts[task_id]
        base_w = (1.0 / c) ** args.is_balance_alpha
        extra_w = args.extra_task_weight.get(task_id, 1.0)
        weights.append(base_w * extra_w)
...
def run_training(model, processor, train_loader):
    ...
    logger.info(
        f"Start IS-only refinement | epochs={args.epochs} | batch_size={args.batch_size} | "
    ...
    for epoch in range(args.epochs):
        pbar = tqdm(train_loader, desc=f"IS boost epoch {epoch + 1}/{args.epochs}")
...
def main():
    setup_runtime()
    ensure_dir_exists(args.base_checkpoint_dir, "Base checkpoint directory for IS refinement")
    ...
    if args.mode in ["train", "all"]:
        train_ds = ISOnlyTrainDataset(args.train_jsonl_dir, processor)
